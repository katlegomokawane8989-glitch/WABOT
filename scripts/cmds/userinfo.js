...
module.exports = {
  ...
  onStart: async ({ api, event, message, threadsData, userData }) => {
    try {
      ...
      } else if (u && u.name !== displayName && displayName !== "No Name Found") {
        try {
          await global.GoatBot.DB.users.set(targetJid, displayName, "name");
        } catch (e) { }
      }
      ...
      let buffer = null;
      try {
        const targetUrl = await global.db.usersData.getAvatarUrl(api, targetJid);
