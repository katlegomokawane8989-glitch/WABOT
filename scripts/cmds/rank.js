...
module.exports = {
	...
	onStart: async function ({ message, event, userData, threadsData, commandName, api }) {
		deltaNext = 5; // Default from envConfig


		let targetUsers;
		const arrayMentions = event.mentions || [];
		...
		const rankCards = await Promise.all(targetUsers.map(async userID => {
			const rankCard = await makeRankCard(userID, userData, threadsData, event.threadID, deltaNext, api);
			return { attachment: { stream: rankCard, type: "image" } };
		}));
	...
	onChat: async function ({ userData, event }) {
		if (!event.isGroup) return;
		let exp = await userData.get(event.senderID, "exp", 0);
		if (isNaN(exp) || typeof exp != "number")
			exp = 0;
		try {
			await userData.set(event.senderID, exp + 1, "exp");
		}
		catch (e) { }
	}
};
...
async function makeRankCard(userID, userData, threadsData, threadID, deltaNext, api = global.GoatBot.fcaApi) {
	let exp = await userData.get(userID, "exp", 0);
	...
	const norm = global.normUID || (id => String(id).split("@")[0].split(":")[0]);
	const targetID = norm(userID);
	const rank = allUser.findIndex(user => norm(user.userID) === targetID) + 1;

	const customRankCard = await threadsData.get(threadID, "data.customRankCard", {});
	let avatarUrl = null;
	let displayName = allUser[rank - 1]?.name || "Unknown";
	if (global.utils) {
		if (global.utils.getAvatar) {
			avatarUrl = await global.utils.getAvatar(api, userID);
		}
		if (global.utils.getLiveMemberName && (displayName === "Unknown" || displayName === userID)) {
			displayName = await global.utils.getLiveMemberName(api, userID, userData);
		}
	}
	if (!avatarUrl) avatarUrl = "https://i.ibb.co/1sBtvL0/1322286.png";
	...
	for (const key of checkImagKey) {
		if (!isNaN(configRankCard[key])) {
			try {
				if (api && api.resolvePhotoUrl) configRankCard[key] = await api.resolvePhotoUrl(configRankCard[key]);
			} catch (_) { }
		}
	}
