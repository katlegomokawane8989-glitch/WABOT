...
async function resolveAdminJID(api, uid) { 
  ...
  try { 
    ...
    const lidKey   = bare + "@lid"; 
    const phoneKey = bare + "@s.whatsapp.net"; 
    if (allUsers.some(u => u.userID === lidKey))   return lidKey; 
    if (allUsers.some(u => u.userID === phoneKey)) return phoneKey; 
  } catch (_) { } 
  const isLid = /^[12]\d{14}$/.test(bare); 
  return bare + (isLid ? "@lid" : "@s.whatsapp.net"); 
}

async function sendAttachments(api, event, toJID, quoteObj) {
  let attachments = [];
  let msgObj = null;
  ...
  if (event.attachments && event.attachments.length > 0) {
    ...
    msgObj = {
      key: {
        ...
        participant: event.messageReply.senderID
      },
      message: event.messageReply.raw
    };
  }

  if (!attachments || attachments.length === 0 || !msgObj) return;
  ...
  for (const attachment of attachments) {
    try {
      ...
      const msgOptions = quoteObj ? { replyToMessage: quoteObj } : {};

      if (type === "photo" || type === "image") {
        await api.sendImage(buffer, toJID, caption, msgOptions);
      } else if (type === "video") {
        await api.sendVideo(buffer, toJID, caption, msgOptions);
      } else if (type === "audio" || type === "ptt") {
        await api.sendAudio(buffer, toJID, { ...msgOptions, ptt: type === "ptt" || attachment.ptt, mimetype });
      } else if (type === "sticker") {
        await api.sendSticker(buffer, toJID, msgOptions);
      } else {
        await api.sendDocument(buffer, toJID, attachment.fileName || "file", { ...msgOptions, mimetype });
      }
    } catch (err) {
      console.error("sendAttachments error:", err);
    }
  }
}
  ...
  onStart: async function ({ args, message, event, userData, threadsData, api }) { 
    ...
    if (isGroup) { 
      try { 
        ...
        threadName = thread.threadName || thread.name || "Unknown Group"; 
      } catch (_) { } 
    } 
    ...
    for (const uid of resolvedAdminBot) { 
      ...
      try { 
        ...
        await sendAttachments(api, event, uid);
    ...
    if (msg2) { 
      return message.reply({ body: msg2, mentions: mentionJIDs }); 
    } 
  }, 
    ...
    switch (type) { 
      case "userCallAdmin": { 
        ...
        try { 
          ...
          await sendAttachments(api, event, threadID, quoteObj);
